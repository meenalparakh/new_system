...
def pick_place_plan(grasp, end_grasp):
    ...
    pose_lst = [pre_pose, pose, lift_pose, end_pose, pre_place_pose]
...
class PandaReal:
    ...
    def motion_plan(self, grasp, end_grasp, pcd=None):
        ...
        pick_plan = self.get_plan(
            [self.pb_robot.arm.get_jpos(), pre_jnts, sol_jnts], pcd=pcd
        )
        place_plan = self.get_plan([sol_jnts, lift_jnts, place_jnts], pcd=pcd)
        reset_plan = self.get_plan(
            [place_jnts, pre_place_jnts, self.reset_joints], pcd=pcd
        )
...
class PandaRealPolymetis:
    def __init__(self, robot="franka", viz=None):
        ...
        self.ik_helper = FrankaIK(
            gui=True,
            base_pos=[0, 0, 0],
            occnet=False,
            robotiq=(self.args.gripper_type == "2f140"),
            mc_vis=self.mc_vis,
        )
        # self.world = WorldInterface(viz)
        ...
        gripper_open_pos = (
            ...
            else self.gripper.get_state().max_width
        )
