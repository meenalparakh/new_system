...
def arrange_edges(object_dicts, new_obj_lst):
    for obj_id in new_obj_lst:
        ...
        dist = list(
            np.linalg.norm(
                np.array(centers) - np.array(info["object_center"]).reshape((1, 3)),
                axis=1,
            )
        )
...
def contains_template(objects_name_lst, container_name, voice="active"):
    ...
    if voice == "active":
        ...
        possibles = [
            " " + start + f" {identifier} in " + (container_name) + ".",
            " "
            + start
            + (" lies " if identifier == "is" else " lie ")
            + "in "
            + f"{container_name}.",
        ]
...
def graph_traversal(object_dicts, new_obj_lst, side="right"):
    ...
    start = (
        new_obj_lst[np.argmin(start)]
        if side == "left"
        else new_obj_lst[np.argmax(start)]
    )
    ...
    while len(remaining) > 0:
        while len(branches) != 0:
            ...
            branches = [
                (chosen_obj, e) for e in object_dicts[chosen_obj]["edges"]
            ] + branches
        if len(remaining) > 0:
            remaining_ids = list(remaining)
            centers = np.array(
                [object_dicts[id]["object_center"] for id in remaining_ids]
            )
            last_center = np.array(
                object_dicts[travelled[-1]]["object_center"]
            ).reshape((1, 3))
            next_id = remaining_ids[
                np.argmin(np.linalg.norm(centers - last_center, axis=1))
            ]
...
def get_direction_label(direction_vector, obj_name):
    ...
    precise_description = (
        f"{np.abs(direction)*180/np.pi} degrees from the vertical,"
    ...
    if np.abs(direction) < np.pi / 12:
        ...
        direction_label = "bottom right diagonal"
        return [lst[11], lst[12]]
        ...
        direction_label = "top right diagonal"
        return [lst[11], lst[12]]
    elif np.abs(direction + np.pi / 4) < np.pi / 6:
        direction_label = "bottom left diagonal"
        return [lst[13], lst[14]]
    elif np.abs(direction + np.pi / 2) < np.pi / 12:
        ...
        direction_label = "top left diagonal"
        return [lst[13], lst[14]]
...
def text_description(object_dicts, traversal_order, path, side="right"):
    ...
    first_object_line = (
        f" At the {side} of all the objects on the table lies "
    ...
    while len(path) > 0:
        ...
        direction_vector = np.array(next_object_dict["object_center"]) - np.array(
            source_obj_dict["object_center"]
        )
        ...
        contained_object_names = [
            object_dicts[n]["used_name"]
            for n in next_object_dict["relation"]["contains"]
        ]
        contained_line = contains_template(
            contained_object_names, next_object_name, voice="active"
        )

        above_object_names = [
            object_dicts[n]["used_name"]
            for n in list(next_object_dict["relation"]["below"])
        ]
        ...
        start = (
            start
            + " "
            + capitalize_first_letter(chosen_line)
            + contained_line
            + above_line
        )
    return start
...
def make_options(
    pick_targets, place_targets, options_in_api_form=True, termination_string="done()"
):
    options = []
